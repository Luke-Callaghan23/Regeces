...
void reTest()
{
	...
	while (true)
	{
		...
		if (sLine->len > 2)
		{
			...
			else if ((sLine->data)[0] == 't' && (sLine->data)[1] == 'm' && (sLine->data)[2] == ' ')
			{
				...
				for (int loop = 0; loop < iTableSize; loop++)
				{
					kvPair * targ = table[loop];

					printf("For regex %s:\n", targ->str->data);
					//LinkedList * llSplits = regexSplitAtMatches(targ->data, sRem);

					MatchReport * mrReport = regexGenerateMatchReport(targ->data, sRem);

					
					if (mrReport == error)
					{
						printError(mrReport, -1, -1);
						printf("\n");
						continue;
					}

					LLNode * llnMatchPtr = mrReport->llMatches->front;
					while (llnMatchPtr)
					{
						Match * maCur = llnMatchPtr->data;
						printf("  -   %d-%d   %s\n", maCur->iBeg, maCur->iEnd, maCur->sSubstring->data);
						llnMatchPtr = llnMatchPtr->next;
					}
			...
			else if ((sLine->data)[0] == 't' && (sLine->data)[1] == 't' && (sLine->data)[2] == ' ')
			{
				...
				for (int loop = 0; loop < iTableSize; loop++)
				{
					...
					HashTable * ht = regexGetTaggedSubstrings(targ->data, sRem);
					
					//nr \s*(;type;(\w+)\s)?\s*;var_name;(\w+[\w\d]*`*)\s*=\s*;expression;([^\s].*)\s*\;?\s*
					//nr \s*(;type;(\w+)\s)?\s*;var_name;(\w+[\w\d]*`*)\s*=\s*;expression;(([^\s;]+(\s+[^\s;]+)*|".*"|'.*')[\s*;]*
